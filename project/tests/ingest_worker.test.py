...
def _mock_requests(mocker, sessions=None, drivers=None, laps=None,
                   ...
                   location=None):
    """URL-aware mock for OpenF1. The worker fetches several datasets; laps and
    position are now session-wide (one call), car_data/location are per driver.
    We map each path to a fixed response instead of relying on call order or on
    the query params (the server-side speed/date filters are not simulated)."""
    data_by_path = {
...
def _event(session_key="9158"):
    return {"detail": {"session_key": session_key}}


def test_missing_session_key_returns_400(ingest_worker_mod):
    result = ingest_worker_mod.handler({"detail": {}}, None)
    assert result["statusCode"] == 400
    assert "session_key" in result["body"]


def test_empty_event_returns_400(ingest_worker_mod):
    result = ingest_worker_mod.handler({}, None)
    assert result["statusCode"] == 400


@mock_aws
def test_s3_object_created(ingest_worker_mod, mocker):
    _mock_requests(mocker)
    result = ingest_worker_mod.handler(_event("9158"), None)
...
@mock_aws
def test_response_counts_session_wide(ingest_worker_mod, mocker):
    # laps are now fetched ONCE per session (not per driver), so laps_fetched
    # equals the size of the single laps response regardless of driver count.
    _mock_requests(mocker, drivers=[_DRIVER, _DRIVER], laps=[_LAP])
    result = ingest_worker_mod.handler(_event("9158"), None)
    assert result["statusCode"] == 200
    body = json.loads(result["body"])
    assert body["drivers_fetched"] == 2
    assert body["laps_fetched"] == 1


@mock_aws
def test_empty_sessions_returns_404(ingest_worker_mod, mocker):
    _mock_requests(mocker, sessions=[], drivers=[], laps=[])
    result = ingest_worker_mod.handler(_event("9999"), None)
    assert result["statusCode"] == 404


@mock_aws
def test_openf1_timeout_returns_502(ingest_worker_mod, mocker):
...
@mock_aws
def test_car_data_decimated_to_1hz(ingest_worker_mod, mocker):
...
@mock_aws
def test_location_fetched_only_in_moving_intervals(ingest_worker_mod, mocker):
    # location is fetched only inside intervals derived from car_data (moving
    # points). With no car_data there are no intervals -> no location.
    session = dict(_SESSION,
                   date_start="2023-09-03T13:00:00+00:00",
                   date_end="2023-09-03T13:00:05+00:00")
    location = [
        {"date": "2023-09-03T12:59:59+00:00", "driver_number": 1, "x": 1, "y": 1, "z": 0},
        {"date": "2023-09-03T13:00:02+00:00", "driver_number": 1, "x": 2, "y": 2, "z": 0},
        {"date": "2023-09-03T13:00:10+00:00", "driver_number": 1, "x": 3, "y": 3, "z": 0},
    ]
    _mock_requests(mocker, sessions=[session], location=location)
    result = ingest_worker_mod.handler(_event("9158"), None)
    assert result["statusCode"] == 200
    assert json.loads(result["body"])["location_points"] == 0


@mock_aws
def test_telemetry_clipped_to_race_window(ingest_worker_mod, mocker):
    # car_data defines a moving interval around 13:00:01-13:00:03, so location
    # gets fetched; _reduce_telemetry then clips it to the race window.
    session = dict(_SESSION,
                   date_start="2023-09-03T13:00:00+00:00",
                   date_end="2023-09-03T13:00:05+00:00")
    car_data = [
        {"date": "2023-09-03T13:00:01+00:00", "driver_number": 1, "speed": 100},
        {"date": "2023-09-03T13:00:03+00:00", "driver_number": 1, "speed": 120},
    ]
    location = [
    ...
    _mock_requests(mocker, sessions=[session], car_data=car_data, location=location)
    result = ingest_worker_mod.handler(_event("9158"), None)
    assert result["statusCode"] == 200
